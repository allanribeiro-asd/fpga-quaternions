...
ENTITY calc IS
	PORT (
		...
		chipselect 	 : INSTD_LOGIC;
		regselect: INSTD_LOGIC_VECTOR( 1 DOWNTO 0);
		write_en: INSTD_LOGIC;
		read_en: INSTD_LOGIC;
		writedata	 : INsfixed (0 downto -31);
...
ARCHITECTURE behavior OF calc IS
	...
	write_enable_reg_0 <= write_en and chipselectand (not regselect(1)) and (not regselect(0)); 
	write_enable_reg_1 <= write_en and chipselectand (not regselect(1)) andregselect(0) ;
	write_enable_reg_2 <= r32_o_0(-31);

	writedata_2 <=r32_o_1when ((r32_o_0(-30) = '1') and (r32_o_0(-31) = '1')) else --Load 
